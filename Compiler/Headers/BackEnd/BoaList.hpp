...
BOA_1 ( 3, 0,   BoaNone,                      "",                        {}, {})
BOA_2 ( 4, 2,   BoaPush,                  "push", { cmd_push   (example); }, { i++; SP_ADD PUSH (Digit,     code_[i]) PUSH (Reg,            regist_[code_[i]]) PUSH (Cell,            ram_[code_[i]]) PUSH (Param, data_[regist_[2] - code_[i] - 1]) })
BOA_2 ( 5, 2,    BoaPop,                   "pop", { cmd_pop    (example); }, { i++; SP_SUB POP  ( None, data_.pop ()) POP  (Reg, MOVE (regist_, data_.pop ())) POP  (Cell, MOVE (ram_, data_.pop ())) })
BOA_2 ( 6, 2,    BoaMov,                   "mov", { cmd_mov    (example); }, { i++; regist_[code_[i]] = regist_[code_[i + 1]]; i++; })
BOA_2 ( 7, 1,     BoaIn,                    "in", { cmd_in_out (example); }, { i++; INPUT  (Reg, regist_) INPUT  (Cell, ram_) })
BOA_2 ( 8, 1,    BoaOut,                   "out", { cmd_in_out (example); }, { i++; OUTPUT (Reg, regist_) OUTPUT (Cell, ram_) })
